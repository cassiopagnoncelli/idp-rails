...
module IdpRails
  ...
  class JwksClient
    ...
    BASE_REDIS_KEY = "idp-rails:jwks"

    def initialize(config = IdpRails.configuration)
      super() # MonitorMixin
      @config = config
    ...
    def schedule_background_refresh
      synchronize do
        ...
        @refresh_thread = Thread.new do
          ...
          @config.logger.error("[IdpRails] Background JWKS refresh failed: #{e.message}")
        end
      end
    end
    ...
    def fetch_and_cache
      ...
      if cached_json
        ...
        @config.logger.info("[IdpRails] JWKS loaded from Redis cache: #{keys.size} key(s)")
        return
      end
    ...
    def fetch_from_origin
      ...
      @config.logger.info("[IdpRails] JWKS refreshed from origin: #{keys.size} key(s)")
    end
    ...
    def read_from_redis
      ...
      @config.logger.warn("[IdpRails] Redis read failed: #{e.message}")
      nil
    end
    ...
    def write_to_redis(jwks_json)
      ...
      @config.logger.warn("[IdpRails] Redis write failed: #{e.message}")
    end
